...
int main() {
    ...
    const int *a1 = &b;
    //常量指针:指针的地址可更改,内容为常量,声明时可以不初始化
    int* const a2 = &b;
    //指针常量:指针的内容可更改,指针的地址为常量,声明时必须初始化
...
**常量指针**的本质是指针，指针内容是一个**常量**的地址。所以指针存放的常量的地址可更改，对应的内容不可更改。但可以修改指针存放的地址来修改常量。

**指针常量**的本质是常量，因此指针的地址是常量，不可更改，而指针的内容可以更改。
